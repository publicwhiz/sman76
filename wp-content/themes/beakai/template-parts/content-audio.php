...
$beakai_audio_url = function_exists('get_field') ? get_field( 'fromate_style' ) : NULL;
$categories = get_the_terms( $post->ID, 'category' );
if( is_single() ): ?>
    <article id="post-<?php the_ID(); ?>" <?php post_class('postbox post format-audio mb-50'); ?>>
        <?php 
        if( !empty($beakai_audio_url) ) : ?>
            <div class="postbox__audio embed-responsive embed-responsive-16by9 mb-30">
                <?php echo wp_oembed_get( $beakai_audio_url ); ?>
            </div>
        <?php 
        endif; ?>
